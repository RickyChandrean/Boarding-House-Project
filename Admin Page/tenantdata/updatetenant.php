...
if( isset($_POST["submit"]) ){
    ...
    $tenantemrcp = mysqli_real_escape_string($conn,$_POST["tenant_emergcp"]);
    $tenantemrcpph= mysqli_real_escape_string($conn,$_POST["tenant_emergcp_phone"]);
    $tenantemrcpeml= mysqli_real_escape_string($conn,$_POST["tenant_emergcp_email"]);
...
<head>
  ...
  <link rel="stylesheet" href="css/updatetenant.css">
  <title>Update Tenant Databases</title>
</head>

<body>
  <h2 class="title">Update Tenant Database</h2> <br><br>
  ...
  <form action="" method="post">
    <form>
      <div class="mb-3">
        <!--Structure-->
        <!--Q1-->
        <div class="ruangtable">
          <label for="tenant_id" class="form-label">Tenant ID</label>
          <input type="text" name="tenant_id" class="form-control" id="tenant_id" required
            value="<?= $qdr["tenant_id"];?>" aria-describedby="emailHelp">
        </div>

        <!--Q2-->
        <div class="ruangtable">
          <label for="tenant_name" class="form-label">Tenant Name</label>
          <input type="text" name="tenant_name" class="form-control" id="tenant_name" required
            value="<?= $qdr["tenant_name"];?>" aria-describedby="emailHelp">
        </div>

        <!--Q3-->
        <div class="ruangtable">
          <label for="tenant_address" class="form-label">Tenant Address</Address></label>
          <input type="text" name="tenant_address" class="form-control" id="tenant_address" required
            value="<?= $qdr["tenant_address"];?>" aria-describedby="emailHelp">
        </div>

        <!--Q4-->
        <div class="ruangtable">
          <label for="tenant_ktp_no" class="form-label">Tenant KTP Number</label>
          <input type="text" name="tenant_ktp_no" class="form-control" id="tenant_ktp_no" required
            value="<?= $qdr["tenant_ktp_no"];?>" aria-describedby="emailHelp">
        </div>

        <!--Q5-->
        <div class="ruangtable">
          <label for="tenant_phone" class="form-label">Tenant Phone Number</label>
          <input type="text" name="tenant_phone" class="form-control" id="tenant_phone" required
            value="<?= $qdr["tenant_phone"];?>" aria-describedby="emailHelp">
        </div>

        <!--Q6-->
        <div class="ruangtable">
          <label for="tenant_email" class="form-label">Tenant Email</label>
          <input type="text" name="tenant_email" class="form-control" id="tenant_email" required
            value="<?= $qdr["tenant_email"];?>" aria-describedby="emailHelp">
        </div>
        <!--Q10-->
        <div class="ruangtable">
          <label for="tenant_bankaccount" class="form-label">Tenant Bank Account</label>
          <input type="text" name="tenant_bankaccount" class="form-control" id="tenant_bankaccount" required
            value="<?= $qdr["tenant_bankaccount"];?>" aria-describedby="emailHelp">
        </div>

        <!--Q11-->
        <div class="ruangtable">
          <label for="tenant_bankaccount_no" class="form-label">Tenant Bank Account Number</label>
          <input type="text" name="tenant_bankaccount_no" class="form-control" id="tenant_bankaccount_no" required
            value="<?= $qdr["tenant_bankaccount_no"];?>" aria-describedby="emailHelp">
        </div>
